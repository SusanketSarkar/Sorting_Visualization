...
def quick_sort(data, start, end, drawData, timeTick):
    if start < end:
        ...
        drawData(data, [BLUE if x >= start and x < pivot_position else DARK_YELLOW if x == pivot_position
                        else DARK_YELLOW if x > pivot_position and x <=end else DARK_GREEN for x in range(len(data))])
        time.sleep(timeTick)
